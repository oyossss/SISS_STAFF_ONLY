...
- `optional` 입력값이 `mypage`의 티켓의 CSS로 반영됨 > css injection임을 알 수 있습니다
- CSP로 인해 외부로 요청을 보낼 수 없습니다
- `input#staff_code[value^=...]` 조건부 선택자를 이용해 prefix가 맞으면 css injection을 이용해서 의도적으로 탭 크래쉬를 유발시키고 이로 인해서 브라우저 렌더링이 지연됩니다 (HINT:크래쉬는 타임아웃을 유발할 수 있습니다.)
- 참가자는 실행 시간의 차이로 value의 참/거짓을 판별합니다
